...
parser.add_argument('--save_dir', default='./draw_result', type=str, help='Save path')
...
if 'GOT' in args.dataset:
    ...
    dataset_root = os.path.join(args.dataset_dir)
...
for v_idx, video in enumerate(dataset):
    ...
    for P in args.trackers:
        # video names for lower
        txt_name = str(video.name) + '.txt'
        traj_file = os.path.join(args.tracker_result_dir, P, str(video.name) + ".txt")
        if not os.path.exists(traj_file):
            #video.name = video.name[0].lower() + video.name[1:]
            txt_name = str(video.name[0].lower()) + str(video.name[1:]) + '.txt'
        ...
        if "GOT" in args.dataset:
            ...
            try:
                bboxes.append(pd.read_csv(os.path.join(args.tracker_result_dir, P, str(txt_name)), sep='\t|,| ',
                                              header=None, names=['xmin', 'ymin', 'width', 'height'],engine='python'))
                names.append(P.split('/')[-1])
            except:
                try:
    ...
    for idx, (img, gt_bbox) in enumerate(video):
        ...
        plt.text(40,60, "#{:06d}".format(idx),fontdict={'color': 'yellow','size': 20})
        plt.axis('off')
